...
author: "Y. Timmer, F.J.A. Menninga and J. Duiker"
date: "`r Sys.Date()`"
...
figure: Fig.  Overview of pipeline steps, including the pre-processing steps that were not performed by us (gray) and the steps that were executed by us (blue). 
Overview of pipeline steps, including the pre-processing steps that were not performed by us (gray) and the steps that were executed by us (blue). 
...
Table 1 shows a decrease in total reads from ~20K to ~18K in the influent. The mean read length is around 1.4K bases and the GC content is 53%.  The effluent contains 2.16K reads after filtering with Fastplong with a mean length of 1.32K bases. The GC content is 53%. 
...
![]("/home/floris/Documenten/git_repo/metagenomics-ACFC/analysis/Logbook_Yamila/img/krona_in_lagoon.png")
